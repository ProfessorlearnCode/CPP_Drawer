...
class library{
	private:
		struct Node
		{
			...
			Node *next_add;			//Pointer which points to the next node in link
		};
	public:
		Node *head = NULL;			//Declaring and initializing a pointer variable named head 
		void menu();
		void insert();
		void search();
		void update();
...
void library::menu()
{
    ...
    cout<<"\n\n\t\t\t============================================"<<endl;
        ...
        getch();			//Used to wait for user input before displaying the menu again
        goto p;
}
 ...
 void library::insert()
    {
        ...
        cout<<"\n\n\t\t\t=        Library Management System         ="<<endl;
        cout<<"\n\n\t\t\t============================================"<<endl;
        Node *new_node =new Node;		//"new" Keyword to create a new node
        ...
        new_node -> next_add = NULL;	//intializing the pointer
        if(head==NULL)
        {
        	head = new_node;
		}
		...
		{
			Node *ptr = head;		//Temperory Pointer ptr is made
	...
	void library::del()
	{
		...
		else
		{
			...
			if(t_id == head -> id)
			{
				Node *ptr = head;
				head = head -> next_add;
				delete ptr;				//Ptr will delete the respective Book id, "Delete" is a keyword
